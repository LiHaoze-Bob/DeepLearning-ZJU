...
#let _report-value(value) = if value == none { [] } else { value }

#let _report-line(value) = block(
  width: 100%,
  inset: (top: 0.28em, bottom: 0.28em, left: 0.2em, right: 0.2em),
  stroke: (bottom: 0.5pt + black),
  align(center + horizon, _report-value(value)),
)

#let _report-metadata-header(
  course: none,
  experiment-type: "综合",
  name: none,
  author: none,
  ...
  v(3.75cm)
  align(center)[
    #text(size: 15pt, weight: "bold")[浙江大学实验报告]
  ]
  v(0.55em)
  {
    set text(size: 12pt)
    grid(
      columns: (auto, 1fr, auto, 0.34fr),
      column-gutter: 0.28em,
      align: left + horizon,
      [课程名称：], _report-line(course),
      [实验类型：], _report-line(experiment-type),
    )
    v(0.5em)
    grid(
      columns: (auto, 1fr),
      column-gutter: 0.28em,
      align: left + horizon,
      [实验项目名称：], _report-line(name),
    )
    v(0.5em)
    grid(
      columns: (auto, 0.78fr, auto, 0.78fr, auto, 0.78fr),
      column-gutter: 0.28em,
      align: left + horizon,
      [学生姓名：], _report-line(author),
      [专业：], _report-line(major),
      [学号：], _report-line(school-id),
    )
    v(0.5em)
    grid(
      columns: (auto, 0.72fr, auto, 0.92fr, auto, 0.62fr),
      column-gutter: 0.28em,
      align: left + horizon,
      [实验地点：], _report-line(place),
      [实验日期：], _report-line(date),
      [指导老师：], _report-line(teacher),
    )
  }
  v(1.1em)
}
...
#let project(
  ...
  set page(
    paper: "a4",
    margin: (left: 3.175cm, right: 3.175cm, top: 2.54cm, bottom: 2.54cm),
    numbering: if (theme == "lab" or theme == "exp") { none } else { "1" },
    number-align: center,
  )
  ...
  if (theme == "lab" or theme == "exp") {
    ...
    {
      set align(center)
      block(width: 58%, asset-zju-banner)
      v(1.25cm)
      text(size: 16pt, weight: "bold")[本科实验报告]
    }
    v(1.7fr)
    let rows = ()
    if (cover_info == none) {
    ...
    v(1.2cm)
    align(center)[#text(size: 14pt)[#_report-value(date)]]
    v(1fr)
    pagebreak()
  } else if (theme == "project") {
    v(1fr)
